#!/usr/bin/env python3
"""Tiny command-line tool for the Spotify Playlist Manager wrapper.

Usage (from the dev shell):

    python cli.py whoami
    python cli.py playlists
    python cli.py create "New Playlist" --description "hi" --private
    python cli.py show "New Playlist"
    python cli.py search "daft punk"
    python cli.py add "New Playlist" "https://open.spotify.com/track/4uLU6hMCjMI75M1A2tKUQC"
    python cli.py remove "New Playlist" "spotify:track:7ouMYWpwJ422jRcDASZB7P"

Authenticates against SPOTIFY_* environment variables (see .env.example).
"""
...
def cmd_stream(mgr: PlaylistManager, args: argparse.Namespace) -> None:
    ...
    def device_id() -> str:
        ...
        if not dev or not dev.get("id"):
            raise SystemExit(f"device {args.name!r} not found. Start it first: python cli.py stream start --name {args.name}")
        return dev["id"]
    ...
    if args.action == "stop":
        # Stop the active device by transferring away is not possible for a
        # device we don't own the process for. For a managed device just nudge.
        dev = mgr.find_device(args.name)
        if dev:
